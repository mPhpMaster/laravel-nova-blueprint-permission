...
abstract class Field extends FieldElement implements JsonSerializable, Resolvable
{
     ...
     *
     * @param Closure|\Stringable|string|\Illuminate\Contracts\Validation\Rule|\Illuminate\Contracts\Validation\InvokableRule|callable ...$rules
     *
     * @return $this
     */
    public function requiredRules(...$rules): static
    {
        return $this->requiredRule(true, ...$rules);
    }
     ...
     * @return $this
     */
    public function requiredRule($callback = true, ...$rules): static
    {
        $_callback = boolval($callback instanceof Closure ? $callback(getNovaRequest()) : $callback);
        $this->required($callback);

        if (empty($this->rules)) {
            $_callback && $this->rules('required');
        } else {
            $rules = [
                ...array_wrap(
                    array_filter($this->rules, fn($v) => !starts_with(strtolower($v), $_callback ? 'required' : 'nullable'))
                ),
                ...$rules
            ];
            $_callback && ($rules[] = 'required');
     ...
     * @return $this
     */
    public function nullableRules(...$rules): static
    {
        return $this->nullableRule(true, null, ...$rules);
    }
	 ...
	 * @return $this
	 */
	public function requiredRuleCreation($rules = 'required')
	{
	 ...
	 * @return $this
	 */
	public function requiredRuleUpdate($rules = 'required')
	{
	 ...
	 * @return $this
	 */
	public function nullableRuleCreation($rules = 'nullable')
	{
		$this->creationRules($rules);
	 ...
	 * @return $this
	 */
	public function nullableRuleUpdate($rules = 'nullable')
	{
		$this->updateRules($rules);
     ...
     * @return $this
     */
    public function nullableRule($callback = true, $values = null, ...$rules): static
    {
        $callback = boolval($callback instanceof Closure ? $callback() : $callback);
        $this->nullable($callback, $values);

        if (empty($this->rules)) {
            $callback && $this->rules('nullable');
        } else {
            $rules = [
                ...array_wrap(
                    array_filter($this->rules, fn($v) => !starts_with(strtolower($v), $callback ? 'nullable' : 'required'))
                ),
                ...$rules
            ];
            $callback && ($rules[] = 'nullable');
            $rules = array_unique($rules);
            $this->rules($rules);
        }
