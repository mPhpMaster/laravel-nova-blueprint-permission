...
class Nova
{
    ...
    public static function registerFakeLicenseRoute()
    {
        !Route::has('license-check-4-nova') &&
        Route::post('nova-api/license-check', function(Request $request) {
            return response([
                                'data' => $request->all(),
                                'message' => 'Message',
                            ], 204);
        })->name('license-check-4-nova');
    }
    ...
    {
        return Http::post(route('license-check-4-nova'), [
            'url' => request()->getHost(),
